 ...
 *  SDL_ERREXIT:    Error exit.
 */
uint32_t sdl_state_transition(SDL_CONTEXT *context,
                              SDL_STATE action,
                              SDL_YYLTYPE *loc)
{
    uint32_t retVal = SDL_NORMAL;
    ...
    if (context->processingEnabled == true)
    {
        ...
        if (trace == true)
        {
            printf("%s:%d:sdl_state_transition(state: %d, action: %d)\n",
                   __FILE__,
                   __LINE__,
                   context->state,
                   action);
...
int64_t sdl_offset(SDL_CONTEXT *context, int offsetType, SDL_YYLTYPE *loc)
{
    ...
    if (context->processingEnabled == true)
    {
        ...
        {
            (void) sdl_aggregate_member(context,
                                        NULL,
                                        SDL_K_TYPE_NONE,
                                        SDL_K_TYPE_NONE,
                                        loc,
        ...
        {
            myAgg = sdl_get_aggregate(&context->aggregates,
...
int sdl_dimension(SDL_CONTEXT *context, size_t lbound, size_t hbound)
{
    ...
    if (context->processingEnabled == true)
    {
        ...
        if (trace == true)
        {
            printf("%s:%d:sdl_dimension(%ld:%ld)\n",
                   __FILE__,
                   __LINE__,
                   lbound,
                   hbound);
 ...
 *  SDL_ERREXIT:    Error exit.
 */
uint32_t sdl_add_option(SDL_CONTEXT *context,
...
uint32_t sdl_precision(SDL_CONTEXT *context, int64_t precision, int64_t scale)
{
    ...
    if (context->processingEnabled == true)
    {
        ...
        if (trace == true)
        {
            printf("%s:%d:sdl_precision(%ld, %ld)\n",
                   __FILE__,
                   __LINE__,
                   precision,
                   scale);
        ...
        if ((item >= SDL_K_BASE_TYPE_MIN) && (item <= SDL_K_BASE_TYPE_MAX))
        {
            switch (item)
            {
                ...
                case SDL_K_TYPE_HW_INT:
                    if (context->argument[ArgWordSize].value == 32)
                    {
                        retVal = sizeof(int32_t);
                    }
                    else
                ...
                case SDL_K_TYPE_ENTRY:
                    retVal = context->argument[ArgWordSize].value / 8;
                    break;
...
static void _sdl_push_state(SDL_CONTEXT *context, SDL_STATE state)
{
    ...
    if (context->stateIdx >= context->stateSize)
    {
        ...
        context->stateStack = (SDL_STATE *) sdl_realloc(context->stateStack,
                                                        newSize);
        context->stateSize++;
    }
