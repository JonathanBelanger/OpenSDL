...
static SDL_MSG_ARRAY sdlmsg[] =
{
    ...
    {
        "INVSYMDEF",
        "Invalid symbol %.*s specified in -s|--symbol qualifier",
        1,
        0
    },
    {
    ...
    {
        "INVALIGN",
        "Illegal value for -a|--align qualifier in command line",
        0,
        0
    },
    {
    ...
    {
        "DUPLISTQUAL",
        "'-L|--list' qualifier specified more than once on the command line",
        0,
        0
    },
    {
...
uint32_t sdl_get_message(SDL_MSG_VECTOR *msgVector, char **msgStr)
{
    ...
    *msgStr = rtnMsgStr;
    ...
    while ((done == false) && (retVal == SDL_NORMAL))
    {
        ...
        if (rtnMsgStr != NULL)
        {
            msgStrLen += sprintf(&rtnMsgStr[msgStrLen],
                                 "%c%s-%c-%s, %s\n",
                                 first,
                                 facStr,
                                 mySeverity,
