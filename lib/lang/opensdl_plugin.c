...
static char *_sdl_shared_image_name_fmt = "%s%copensdl_%s.%s";
...
uint32_t sdl_load_plugin(char *lang, char **fileExt, uint32_t *langId)
{
     ...
     */
    sprintf(pathToSharedLib, _sdl_shared_image_name_fmt, PATH_SEP, lang);
        ...
        *(void **) &onLoad = dlsym(dlFP, "onLoad");
        ...
        else
        {
            ...
            if (retVal == SDL_NORMAL)
            {
                 ...
                 */
                _sdl_plugin_info = realloc(_sdl_plugin_info,
                ...
                if (_sdl_plugin_info != NULL)
                {
                    ...
                    _sdl_plugin_info[index].lang = strdup(lang);
                    ii = 0;
