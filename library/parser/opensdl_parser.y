...
comments
    : t_line_comment {
            SDL_CALL(sdl_comment_line(&context, $1, (SDL_YYLTYPE *) &@1), @$);
        }
    | t_block_comment {
            SDL_CALL(sdl_comment_block(&context, $1, (SDL_YYLTYPE *) &@1), @$);
        }
    ;
...
module
    : SDL_K_MODULE t_name { 
            SDL_CALL(sdl_state_transition(&context,
                                          Module,
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_module(&context, $2, NULL, (SDL_YYLTYPE *) &@2), @$);
        }
    | SDL_K_MODULE t_name SDL_K_IDENT t_string {
            SDL_CALL(sdl_state_transition(&context,
                                          ...
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_module(&context, $2, $4, (SDL_YYLTYPE *) &@2), @$);
        }
    ;
...
end_module
    : SDL_K_END_MODULE _t_module_name {
            SDL_CALL(sdl_state_transition(&context,
                                          ...
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_module_end(&context, $2, (SDL_YYLTYPE *) &@2), @$);
        }
    ;
...
_v_expression
    : _v_expression SDL_K_PLUS _v_terminal { $$ = $1 + $3; }
    | _v_expression SDL_K_MINUS _v_terminal { $$ = $1 - $3; }
    | _v_terminal
    ;

_v_terminal
    : _v_terminal SDL_K_MULT _v_boolean { $$ = $1 * $3; }
...
_v_boolean
    : _v_boolean SDL_K_AND _v_bit_op { $$ = $1 & $3; }
    | _v_boolean SDL_K_OR _v_bit_op { $$ = $1 | $3; }
    | _v_bit_op
    ;
...
_v_factor
    : SDL_K_MINUS _v_factor { $$ = -$2; }
    | SDL_K_NOT _v_factor { $$ = ~$2; }
    | SDL_K_OPENP _v_expression SDL_K_CLOSEP { $$ = $2; }
    | _v_number { $$ = $1; }
    | t_string { SDL_CALL(sdl_str2int(sdl_unquote_str($1), &$$), @$); }
    ;

_v_number
    : v_int { $$ = $1; }
    | t_variable { SDL_CALL(sdl_get_local(&context, $1, &$$), @$); }
    | t_hex { sscanf($1, "%lx", &$$); sdl_free($1); }
    | t_octal { sscanf($1, "%lo", &$$); sdl_free($1); }
    | t_binary { $$ = sdl_bin2int($1); }
    | t_ascii { $$ = (__int64_t) $1[0]; sdl_free($1); }
    | SDL_K_DOT {
            $$ = sdl_offset(&context, SDL_K_OFF_BYTE_REL, (SDL_YYLTYPE *) &@1);
        }
    | SDL_K_FULL {
            $$ = sdl_offset(&context, SDL_K_OFF_BYTE_BEG, (SDL_YYLTYPE *) &@1);
        }
    | SDL_K_CARAT {
...
varset
    : t_variable SDL_K_EQ _v_expression{
            SDL_CALL(sdl_state_transition(&context,
                                          ...
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_set_local(&context, $1, $3, (SDL_YYLTYPE *) &@1), @$);
        }
    ;
...
literal
    : SDL_K_LITERAL
    | t_literal_string {
        SDL_CALL(sdl_literal(&context, &literal, $1, (SDL_YYLTYPE *) &@1), @$);
        }
    | SDL_K_END_LITERAL {
        SDL_CALL(sdl_literal_end(&context, &literal, (SDL_YYLTYPE *) &@1), @$);
        }
    ;
...
declare
    : SDL_K_DECLARE _t_id SDL_K_SIZEOF _v_sizeof { 
            SDL_CALL(sdl_state_transition(&context,
                                          ...
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_declare(&context, $2, $4, (SDL_YYLTYPE *) &@2), @$);
        }
    ;

_v_sizeof
    : SDL_K_OPENP _v_expression SDL_K_CLOSEP { $$ = $2 * SDL_K_SIZEOF_MIN; }
    | _v_datatypes { $$ = $1; }
    ;

prefix
    : SDL_K_PREFIX _t_id {
            SDL_CALL(sdl_add_option(&context,
                                    Prefix,
                                    0,
                                    $2,
                                    (SDL_YYLTYPE *) &@2),
...
_v_basetypes
    ...
    | SDL_K_CHAR SDL_K_LENGTH SDL_K_MULT { $$ = SDL_K_TYPE_CHAR_STAR; }
    | _v_address { $$ = $1; }
    | SDL_K_ANY { $$ = SDL_K_TYPE_ANY; }
    | SDL_K_BOOL { $$ = SDL_K_TYPE_BOOL; }
    ;

_v_char_opt
    : %empty { $$ = SDL_K_TYPE_CHAR; }
    | SDL_K_VARY { $$ = SDL_K_TYPE_CHAR_VARY; }
    ;

_v_integer
    : SDL_K_BYTE _v_signed { $$ = SDL_K_TYPE_BYTE * $2; }
    | SDL_K_INT_B _v_signed { $$ = SDL_K_TYPE_INT_B * $2; }
    | SDL_K_WORD _v_signed { $$ = SDL_K_TYPE_WORD * $2; }
    | SDL_K_INT_W _v_signed { $$ = SDL_K_TYPE_INT_W * $2; }
    | SDL_K_LONG _v_signed { $$ = SDL_K_TYPE_LONG * $2; }
    | SDL_K_INT _v_signed { $$ = SDL_K_TYPE_INT * $2; }
    | SDL_K_INT_L _v_signed { $$ = SDL_K_TYPE_INT_B * $2; }
    | SDL_K_INT_HW _v_signed { $$ = SDL_K_TYPE_INT_HW * $2; }
    | SDL_K_HW_INT _v_signed { $$ = SDL_K_TYPE_HW_INT * $2; }
    | SDL_K_QUAD _v_signed { $$ = SDL_K_TYPE_QUAD * $2; }
    | SDL_K_INT_Q _v_signed { $$ = SDL_K_TYPE_INT_Q * $2; }
    | SDL_K_OCTA _v_signed { $$ = SDL_K_TYPE_OCTA * $2; }
    ;

_v_float
    : SDL_K_TFLOAT { $$ = SDL_K_TYPE_TFLT; }
    | SDL_K_TFLOAT SDL_K_COMPLEX { $$ = SDL_K_TYPE_TFLT_C; }
    | SDL_K_SFLOAT { $$ = SDL_K_TYPE_SFLT; }
    | SDL_K_SFLOAT SDL_K_COMPLEX { $$ = SDL_K_TYPE_SFLT_C; }
    | SDL_K_XFLOAT { $$ = SDL_K_TYPE_XFLT; }
    | SDL_K_XFLOAT SDL_K_COMPLEX { $$ = SDL_K_TYPE_XFLT_C; }
    | SDL_K_FFLOAT { $$ = SDL_K_TYPE_FFLT; }
    | SDL_K_FFLOAT SDL_K_COMPLEX { $$ = SDL_K_TYPE_FFLT_C; }
    | SDL_K_DFLOAT { $$ = SDL_K_TYPE_DFLT; }
    | SDL_K_DFLOAT SDL_K_COMPLEX { $$ = SDL_K_TYPE_DFLT_C; }
    | SDL_K_GFLOAT { $$ = SDL_K_TYPE_GFLT; }
    | SDL_K_GFLOAT SDL_K_COMPLEX { $$ = SDL_K_TYPE_GFLT_C; }
    | SDL_K_HFLOAT { $$ = SDL_K_TYPE_HFLT; }
    | SDL_K_HFLOAT SDL_K_COMPLEX { $$ = SDL_K_TYPE_HFLT_C; }
...
_v_object
    : %empty { $$ = 0; }
    | _v_datatypes { $$ = $1; }
    | SDL_K_OPENP SDL_K_ENTRY _basealign SDL_K_CLOSEP { $$ = SDL_K_TYPE_ENTRY; }
    | SDL_K_OPENP _v_datatypes _basealign SDL_K_CLOSEP { $$ = $2; }
    ;
...
aggregate
    : SDL_K_AGGREGATE _t_id SDL_K_STRUCTURE _v_aggtypes {
            SDL_CALL(sdl_state_transition(&context,
                                          ...
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_aggregate(&context,
                                   $2,
                                   $4,
                                   SDL_K_TYPE_STRUCT,
                                   (SDL_YYLTYPE *) &@2),
    ...
    | SDL_K_END _t_aggr_id SDL_K_SEMI {
            SDL_CALL(sdl_state_transition(&context,
                                          ...
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_aggregate_compl(&context, $2, (SDL_YYLTYPE *) &@2),
                     @$);
       }
    | SDL_K_END SDL_K_SEMI {
            SDL_CALL(sdl_state_transition(&context,
                                          ...
                                          (SDL_YYLTYPE *) &@1),
                     @$);
            SDL_CALL(sdl_aggregate_compl(&context, NULL, (SDL_YYLTYPE *) &@1),
                     @$);
       }
...
aggregate_body
    : _t_aggr_id _v_datatypes {
            SDL_CALL(sdl_aggregate_member(&context,
                                          $1,
                                          $2,
                                          SDL_K_TYPE_NONE,
                                          (SDL_YYLTYPE *) &@1,
...
entry
    : SDL_K_ENTRY _t_id entry_options {
            SDL_CALL(sdl_state_transition(&context, Entry, (SDL_YYLTYPE *) &@1),
                     @$);
...
parameter_options
    : _v_datatypes _v_passing_option parameter_choices {
            SDL_CALL(sdl_add_parameter(&context, $1, $2, (SDL_YYLTYPE *) &@1),
                     @$);
        }
    ;

_v_passing_option
    : %empty { $$ = SDL_K_PARAM_NONE; }
    | SDL_K_VALUE { $$ = SDL_K_PARAM_VAL; }
    | SDL_K_REF { $$ = SDL_K_PARAM_REF; }
    | SDL_K_DESC { $$ = SDL_K_PARAM_REF; }
    | SDL_K_STR_DESC { $$ = SDL_K_PARAM_REF; }
    ;
...
parameter_loop
    : SDL_K_IN { sdl_add_option(&context, In, 0, NULL, (SDL_YYLTYPE *) &@1); }
    | SDL_K_OUT { sdl_add_option(&context, Out, 0, NULL, (SDL_YYLTYPE *) &@1); }
    | _a_named { sdl_add_option(&context, Named, 0, $1, (SDL_YYLTYPE *) &@1); }
    | SDL_K_DIMENSION _v_expression {
            SDL_CALL(sdl_add_option(&context,
                                    Dimension,
                                    $2,
...
conditionals
    : SDL_K_IFLANG language_list {
            SDL_CALL(sdl_conditional(&context,
                                     SDL_K_COND_LANG,
                                     sdl_get_language(&context,
                                                      (SDL_YYLTYPE *) &@2),
                                     (SDL_YYLTYPE *) &@1),
                     @$);
        }
...
language_list
    : %empty
    | language_list _t_id {
            SDL_CALL(sdl_add_language(&context, $2, (SDL_YYLTYPE *) &@2), @$);
        }
    ;
